...
static size_t count_different( ForwardIterator begin, const ForwardIterator end )
{
	if( begin == end )
		return 0;
	size_t count = 1;
	for( typename ForwardIterator::value_type previous = *begin, current = previous;
		 begin != end;
		 previous = current, current = *begin )
	{
		++begin;
		if( current != previous )
			++count;
...
struct Pattern
{
	...
	void prepare()
	{
		...
		for( Values::const_iterator vItr = allValues.begin(), vEnd = allValues.end();
			 vItr != vEnd;
			 ++vItr, ++itr )
		{
			if( itr == end )
	...
	void bakeConstantLocations()
	{
		...
		for( LocationDatas::const_iterator itr = locationData.begin(), end = locationData.end();
			 itr != end;
			 ++itr )
		{
...
struct Splitter
{
		...
		pivot    ( std::min_element( begin, end, &LocationData::less ) )
	{
		...
		size_t i = 0;
		Values tmp;
		for( Values::const_iterator pValue = dispatcher.begin(), pValueEnd = dispatcher.end();
			 pValue != pValueEnd;
			 ++pValue, ++i)
		{
			tmp.clear();
	...
	Pattern& getPattern( value_type value )
	{
		...
		if( pFound == map.end() )
		{
			...
			for( LocationsPtr::const_iterator itr = pColumns.begin(), end = pColumns.end();
				 itr != end;
				 ++itr )
